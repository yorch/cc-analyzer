...
Between the search box and the stat cards sits the **Weekly digest** card (`WeeklyDigestCard`), a compact read of `/api/report`: the period label with the period it is compared against, then four cards — cost with its signed delta, sessions with theirs, the week's top project, and the correction share. A zero-session period renders "No sessions in this period" instead of the cards; it is a legitimate answer, not an error. A **Copy as markdown** button writes the full report to the clipboard using `buildDigestMarkdown` imported straight from the bun-free `src/core/digest.ts` — the same function `cc-analyzer report --md` calls — so the pasted text is byte-identical to the CLI's and no extra endpoint or `?format=` branch exists on the server. The card's own subtitle carries the scoping note (sessions are attributed to their start day). The full digest, including models, cache, reliability, skills, and the insight snapshot, lives in `cc-analyzer report`.
