...
- [src/web/server.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts)
- [src/web/api.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts)
- [src/web/spa.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/spa.ts)
- `scripts/compile-with-spa.ts`

## Overview

The Web Server & API subsystem is the backend of `cc-analyzer serve`: a local [Hono](https://hono.dev) application that exposes the analytics core over a JSON HTTP API and serves the single-page application (SPA) that renders it. It is the third of the three frontends over `src/core/`, alongside the command-line interface (CLI) and the terminal UI (TUI), and it is the only one that reaches the browser. The entry point `runServe()` opens the SQLite index, loads pricing, composes the app, and binds a `Bun.serve` socket on port `4317` by default ([src/web/server.ts#L81-L106](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts#L81-L106)).

The subsystem has three concerns kept in separate modules: request routing and read-only data access ([src/web/api.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts)), app composition with a loopback security guard and SPA fallback ([src/web/server.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts)), and the embedded SPA bytes ([src/web/spa.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/spa.ts)). All three factory functions — `createApi`, `createApp` — are pure over their `db` and `pricing` arguments, so the app can be built and exercised without binding a port ([src/web/api.ts#L45](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L45), [src/web/server.ts#L39-L43](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts#L39-L43)).
...
`runServe()` composes the app once and hands `app.fetch` to `Bun.serve` ([src/web/server.ts#L92-L95](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts#L92-L95)). `createApp` mounts the Host-header guard first so it wraps every route, mounts the API router at `/`, adds a JSON 404 for unmatched `/api/*` paths, then falls back to the SPA for everything else ([src/web/server.ts#L50-L72](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts#L50-L72)). `createApi` reads exclusively from the SQLite index and the loaded pricing table, delegating all computation to `src/core/` modules.

## Module Layout

| Module | Path | Responsibility |
| ------ | ---- | -------------- |
| `server` | [src/web/server.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts) | `runServe` lifecycle, `createApp` composition, loopback Host guard, SPA fallback |
| `api` | [src/web/api.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts) | `createApi` Hono router: all `/api/*` JSON endpoints with fingerprint memoization |
| `spa` | [src/web/spa.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/spa.ts) | Tracked placeholder used by clean source checkouts |
...
Sources: [src/web/api.ts:L67-L119](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L67-L119) [src/core/inventory.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/inventory.ts) [src/core/setup-audit.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/setup-audit.ts)

### Bulk export endpoint

`GET /api/export` accepts `format=json,csv,md,html|all`, optional `project=<id>`
or indexed `session=<id>`, `redact=1`, `split=1`, and `transcript=1`, then
streams a ZIP when the system `zip` command is available. Project and session
scopes are mutually exclusive; unknown or ambiguous projects return `404`/`409`,
and an unindexed session is not read from an arbitrary path. This route is
unauthenticated, so use the default loopback bind or add your own access control
before exposing the server to a network.

### Project and session endpoints

Project drill-down endpoints scope data to one project id. Stored ids are uniformly root-qualified (`<rootSlug>~<name>`, which is why the separator is URL-path-safe), so every one of these routes resolves `:id` through `resolveIndexedProject()` — the same lenient rule the CLI uses: a full id matches exactly, a bare encoded name resolves when exactly one root holds that project (so a URL bookmarked before qualification keeps working), and a name held by several roots answers `409` with the candidate ids rather than picking one. The trends memo is keyed by the *resolved* id, so a bare and a qualified reference to one project share a slot. `GET /api/projects` lists indexed projects, each carrying the `claudeDir` it was discovered under so the SPA can tell two same-named projects from different roots apart; `GET /api/projects/:id/sessions` lists that project's sessions; `GET /api/projects/:id/files` returns the files Claude touched, hottest first ([src/web/api.ts#L75](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L75), [src/web/api.ts#L121](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L121), [src/web/api.ts#L135](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L135)). `GET /api/projects/:id/trends` returns per-project chart series memoized under a per-id cache key, and it verifies the project exists before touching the memo `Map` so unknown ids `404` rather than growing the keyspace with probed ids ([src/web/api.ts#L127-L132](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L127-L132)).

`GET /api/sessions/search` is registered before `/api/sessions/:id` so the literal segment `search` is not captured as an id; it clamps the `limit` query to the range `1..1000` to avoid SQLite's `LIMIT -1` unlimited behavior and abusive values, returning an empty array for a blank query ([src/web/api.ts#L137-L144](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L137-L144)). The two per-session endpoints re-parse the live `.jsonl` file rather than serving stale index rows: `GET /api/sessions/:id` returns `analyzeSession(...)` plus the session's `projectId` and a server-computed `insights` sibling. The id rides along because `SessionAnalysis` carries only `projectPath` (the raw cwd), which stops being a unique key once two Claude roots hold a project for the same working directory — the SPA resolves the breadcrumb's project by id instead. It costs nothing: the route reads the indexed row through `sessionRowById`, which already returns `path` and `projectId` together for the cost rank — the session-scoped what-if repricing (`sessionWhatIf`, which needs the pricing table) and the session's cost rank among indexed sessions (`sessionCostRank`, which needs the index; `null` when the session isn't indexed) — and `GET /api/sessions/:id/transcript` returns `buildTranscript(...)` ([src/web/api.ts#L157-L171](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L157-L171)). Because the index is a disposable cache, a `readSession` helper swallows parse failures and both endpoints `404` with a "re-run `cc-analyzer index`" hint when the underlying file has been deleted, never crashing into a `500` ([src/web/api.ts#L146-L163](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L146-L163)).

`GET /api/sessions/:id/report` is the shareable export endpoint that mirrors `cc-analyzer analyze --md/--html/--json --out`: `GET /api/sessions/:id/report?format=md|html|json&redact=1&transcript=1`. Like its siblings it re-parses the live `.jsonl` (not the index) and `404`s with the same "re-run `cc-analyzer index`" hint when the file is gone; `?format` defaults to `md` and an unknown value is a `400`. `redact` hides prompt/transcript bodies and sensitive paths via the same `sanitizeFilename` + capped transcript (`600×2000`, sampled chart tables) and extended JSON redaction (`title`/`projectPath`/`filesTouched`/`bashCommands` → counts/empty) that the CLI uses, so `GET …?format=md&redact=1` and `analyze --md --redact` are byte-identical. The filename is `cc-analyzer-<sanitizedId>.md|html|json` with `Content-Disposition: attachment`, so the browser downloads rather than renders injected content. The response follows the server host trust boundary (loopback is the safe default; non-loopback hosts are unauthenticated), `Content-Type` `text/markdown`/`text/html`/`application/json` accordingly, and it is **not** fingerprint-memoized (transcript size makes caching uncapped payloads a DOS risk); the SPA calls it only for Copy/Download.

`POST /api/sessions/:id/analyze` is the API's **second write-shaped route** and its only one that spawns a subprocess: it hands the session to a locally-installed `claude` for a retrospective and **streams** the result back as NDJSON (`application/x-ndjson`, one `AnalysisEvent` per line — `text` deltas, a terminal `result` carrying `total_cost_usd`, or `error`). It resolves the path with `sessionRowById` (`404` unknown id), reads an optional `{ model }` from the body (defaulting to the persisted `analysisModel`), rejects an invalid model with `400` (`isValidModel`), returns `503` with an install hint when `claude` isn't on `PATH`, then parses+analyzes the session and pipes `runClaudeAnalysis(...)` through Hono's `stream()`. Like `/api/prefs` is intended for a single local user on the default loopback bind; a non-loopback --host exposes it without authentication, and it does not modify the reviewed transcript, but the normal Claude process may run hooks and send content to the model provider: Claude reads the `.jsonl` (`--allowedTools Read`, scoped by `--add-dir`) and is never `--resume`d. `createApi` takes an optional `deps` (`resolveClaudeBinary`, `spawn`) so tests drive a fake binary and subprocess without a real install (`src/core/claude-handoff.ts`).

Sources: [src/web/api.ts:L75-L171](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L75-L171)

### SPA embedding

The web UI is baked into the compiled binary as a string. [src/web/spa.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/spa.ts) exports `spaHtml` (the full HTML document) and the `hasSpa` boolean, which the server consults to decide whether to serve the app or a plain-text "not built" notice ([src/web/server.ts#L66-L72](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts#L66-L72)). The committed placeholder ships `spaHtml = ""` and `hasSpa = false`, so the server compiles and runs before the SPA is ever built ([src/web/spa.ts#L1-L5](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/spa.ts#L1-L5)).
...
Sources: [src/web/server.ts:L10-L14](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts#L10-L14) [src/web/server.ts:L90-L95](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/server.ts#L90-L95) [src/web/api.ts:L42](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L42) [src/web/api.ts:L138-L143](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/web/api.ts#L138-L143)
