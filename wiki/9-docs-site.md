...
`gen-fixtures.ts` builds a synthetic `~/.claude`-shaped dataset so the dashboard image is captured against fabricated sessions rather than real user data ([site/scripts/gen-fixtures.ts:L2-L10](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/scripts/gen-fixtures.ts#L2-L10)). It uses a seeded `mulberry32` pseudo-random number generator (PRNG) so regeneration produces identical numbers and stable screenshots ([site/scripts/gen-fixtures.ts:L19-L29](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/scripts/gen-fixtures.ts#L19-L29)). The lossless PNG is converted to an approximately 80 KB WebP for the lazy-loaded landing preview; screenshot-generation commands explicitly disable telemetry.
...
| Mermaid palette | `LazyMermaid.vue` | Light/dark diagram palette selected at render time |
