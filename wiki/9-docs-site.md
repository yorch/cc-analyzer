...
| ------ | ---- | -------------- |
| VitePress config | [site/.vitepress/config.ts](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/config.ts) | Site metadata, base path, Mermaid theming, hand-defined nav and docs sidebar |
| Landing page | [site/index.md](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/index.md) | Composes the three terminal components plus a closing call-to-action |
...
| Landing components | [site/.vitepress/theme/components/](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/theme/components/CommandDeck.vue) | `TerminalHero`, `TerminalModules`, `CommandDeck` |
...
`config.ts` wraps the site config in `withMermaid()` to enable native Mermaid rendering, sets the `/cc-analyzer/` base path required by project Pages, and forces `appearance: "dark"` since the amber CRT is the intended default with the light "print-out" theme one toggle away ([site/.vitepress/config.ts:L1-L15](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/config.ts#L1-L15)). The Mermaid block sets `theme: "base"` with warm-cream node fills and amber-ink borders tuned for the light palette ([site/.vitepress/config.ts:L21-L38](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/config.ts#L21-L38)). The docs sidebar is **defined by hand** under the `/docs/` key, enumerating every wiki page in order — including nested detail pages such as `2.1`–`2.4` under the Core Analysis Engine ([site/.vitepress/config.ts:L47-L81](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/config.ts#L47-L81)). Because the sync script does not touch the sidebar, regenerating the wiki with a different set of pages requires updating this array to match ([site/README.md:L30-L31](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/README.md#L30-L31)). The top nav exposes Home, Install, and Docs, and the `editLink` points contributors back at the source wiki page rather than the generated docs copy ([site/.vitepress/config.ts:L41-L90](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/config.ts#L41-L90)).
...
`site/index.md` uses the `page` layout with a `cc-landing` class and simply drops in three registered components followed by a closing call-to-action linking to Install, Docs, and GitHub ([site/index.md:L1-L22](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/index.md#L1-L22)). Those components are registered globally in the theme entry, which extends `DefaultTheme`, supplies the custom `Layout`, imports the IBM Plex Mono font weights, and loads `custom.css` ([site/.vitepress/theme/index.ts:L15-L23](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/theme/index.ts#L15-L23)). `Layout.vue` wraps the default layout and injects a purely decorative full-viewport CRT overlay — scanlines, vignette, and flicker — through the `#layout-top` slot with pointer events disabled ([site/.vitepress/theme/Layout.vue:L7-L15](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/theme/Layout.vue#L7-L15)).

The visual identity is an "amber phosphor" CRT: amber monochrome on near-black in dark mode and a warm amber-ink-on-cream "print-out" in light mode, defined as CSS custom properties in `custom.css` ([site/.vitepress/theme/custom.css:L1-L67](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/theme/custom.css#L1-L67)). `TerminalHero.vue` renders a wordmark, tagline, and an animated fake terminal that reveals a sample `cc-analyzer stats` boot sequence line-by-line via a staggered `--i` custom property ([site/.vitepress/theme/components/TerminalHero.vue:L6-L63](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/theme/components/TerminalHero.vue#L6-L63)). `TerminalModules.vue` presents the six product capabilities as loadable "modules" with inline line-SVG icons so no external asset requests are made ([site/.vitepress/theme/components/TerminalModules.vue:L1-L41](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/theme/components/TerminalModules.vue#L1-L41)). `CommandDeck.vue` scripts a three-step quick-start session, tagging each token for phosphor coloring (comment, prompt, continuation, output) ([site/.vitepress/theme/components/CommandDeck.vue:L2-L18](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/theme/components/CommandDeck.vue#L2-L18)).
...
`gen-fixtures.ts` builds a synthetic `~/.claude`-shaped dataset so the screenshots in `site/public/screenshots/` are captured against fabricated sessions rather than real user data ([site/scripts/gen-fixtures.ts:L2-L10](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/scripts/gen-fixtures.ts#L2-L10)). It uses a seeded `mulberry32` pseudo-random number generator (PRNG) so regeneration produces identical numbers and stable screenshots ([site/scripts/gen-fixtures.ts:L19-L29](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/scripts/gen-fixtures.ts#L19-L29)). For each of four synthetic projects it emits JSONL sessions whose events mirror the real transcript shape — user prompts, `assistant` API calls with token `usage` and cache-creation breakdowns, and `tool_result` follow-ups — writing them into encoded project directories under `projects/` ([site/scripts/gen-fixtures.ts:L44-L166](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/scripts/gen-fixtures.ts#L44-L166)). Sessions are spread across roughly four months to populate a rich spend-by-month chart, and the app is pointed at the output via `CC_ANALYZER_CLAUDE_DIR` ([site/scripts/gen-fixtures.ts:L168-L188](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/scripts/gen-fixtures.ts#L168-L188), [site/README.md:L34-L45](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/README.md#L34-L45)).
...
| ------- | -------- | ------- |
| `base` | [config.ts:L8](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/config.ts#L8) | Project-Pages URL prefix `/cc-analyzer/` |
...
| `mermaid.themeVariables` | [config.ts:L21-L38](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/.vitepress/config.ts#L21-L38) | Light-mode diagram palette; dark mode repainted in CSS |
...
`GOTCHAS.md` records three non-obvious VitePress and Mermaid pitfalls hit while building the site, each with a fix already applied in the codebase: feature-page icons must be passed as raw SVG strings rather than `{ svg }` objects, Mermaid's dark mode leaks the light `themeVariables` (repaired with CSS scoped under `.dark`), and `vitepress preview` serves stale content-hashed chunks after a rebuild ([site/GOTCHAS.md:L7-L71](https://github.com/yorch/cc-analyzer/blob/51ccd4e/site/GOTCHAS.md#L7-L71)).
