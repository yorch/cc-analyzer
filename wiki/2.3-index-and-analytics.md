...
`stats.ts` also backs the cache-efficiency **insights** view: `cacheWasteByProject` and `cacheWasteBySession` rank projects (and a project's sessions) by *un-amortized cache-write cost* — `Σ cost_cache_write × max(0, 1 − min(1, cache_read / cache_write))`, computed per session then summed — while `cacheSummary` totals it portfolio-wide and `cacheVerdict(ratio)` classifies the read:write ratio as efficient / ok / leaky ([src/core/stats.ts](https://github.com/yorch/cc-analyzer/blob/4d7658d/src/core/stats.ts)).
