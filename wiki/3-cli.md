...
- [src/cli/index.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts)
- [src/cli/format.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/format.ts)
- [src/cli/render.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/render.ts)

## Overview

The Command-Line Interface (CLI) is the scriptable frontend of `cc-analyzer` and the entrypoint of the compiled binary. [src/cli/index.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts) reads `process.argv`, routes the first token to a command handler, and returns a process exit code — the file ends by calling `process.exit(await main())` at [src/cli/index.ts#L279](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L279). Every handler is a thin wrapper over `src/core`: the CLI parses arguments, invokes a core function, and hands the result to a renderer. It performs no analysis, pricing, or indexing itself.

The subsystem has three modules. [src/cli/index.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts) holds the argument router and one `cmd*` function per command. [src/cli/format.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/format.ts) supplies primitive formatters — currency, counts, byte sizes, durations, relative time — plus a `table` layout helper. [src/cli/render.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/render.ts) composes those primitives into full text reports for a single session (`renderSessionSummary`), for portfolio analytics (`renderStats`), for the setup audit (`renderSetupAudit`), for the portfolio insights (`renderPortfolioInsights`), and for the weekly digest (`renderWeeklyDigest`). Passing `--json` on supported commands bypasses the human renderer, but payloads are command-specific and may include derived health, what-if, rank, or coverage fields rather than being raw core objects.
...
`main` splits `process.argv` into a command and the remaining arguments, then delegates to `runCommand`, whose `switch` maps each command string to a handler at [src/cli/index.ts#L209-L266](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L209-L266). Handlers call into `src/core`; only `analyze`, `stats`, `audit`, `insights`, and `report` route their human-readable output through the renderers, which in turn depend on `format.ts`. The `serve` and no-command (TUI) branches use dynamic `import()` so the heavier web and Ink dependencies load only when actually invoked.

## Module Layout

| Module | Path | Responsibility |
| ------ | ---- | -------------- |
| `index` | [src/cli/index.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts) | Binary entrypoint, argv router, and one handler per command |
| `format` | [src/cli/format.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/format.ts) | Primitive text formatters and the aligned `table` helper |
| `render` | [src/cli/render.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/render.ts) | Composes session, portfolio, and setup-audit text reports from core data |

Sources: [src/cli/index.ts:L1-L41](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L1-L41) [src/cli/format.ts:L1-L11](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/format.ts#L1-L11) [src/cli/render.ts:L1-L20](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/render.ts#L1-L20)

## Key Components

### Argument router

`main` first strips the global `--claude-dir=<path>` flag from `process.argv` (applying it through `setClaudeRootsOverride`, so it may appear anywhere on the line and never reaches a handler as a positional), then destructures the remainder into `command` and `rest`, calls `runCommand`, and returns its exit code at [src/cli/index.ts#L268-L277](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L268-L277). `runCommand` derives two shared values before switching: `json` is true when `rest` contains `--json`, and `positional` filters out any argument starting with `--` so handlers can read positional operands cleanly ([src/cli/index.ts#L209-L212](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L209-L212)). Exit codes are meaningful: `0` for success, `1` for a runtime failure such as a missing session or empty index, and `2` for a usage error such as a missing argument or bad flag.

The `switch` recognizes `version`/`--version`/`-v` (prints `VERSION`), `help`/`--help`/`-h` (prints the `HELP` banner), an `undefined` command that launches the Terminal User Interface (TUI), and a `default` case that reports the unknown command, prints help, and returns exit code `2` ([src/cli/index.ts#L247-L265](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L247-L265)). The `HELP` string embeds the running `VERSION` and documents every command with its flags ([src/cli/index.ts#L22-L41](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L22-L41)).

Sources: [src/cli/index.ts:L209-L279](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L209-L279) [src/cli/index.ts:L22-L41](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L22-L41)

### Discovery commands: `projects` and `sessions`

`cmdProjects` calls `listProjects()` from the core discovery module and prints an aligned table of session count and truncated project label, followed by a total count ([src/cli/index.ts#L43-L57](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L43-L57)). A third **claude dir** column appears only when the listed projects span more than one Claude root, since two roots can hold projects whose labels are identical. When no projects exist it returns `0`, printing every directory that was searched and the setting that put it there — the commonest cause of an empty portfolio is a relocated Claude directory, and naming `~/.claude` unconditionally would be a lie. `cmdSessions` requires a `<projectId>` operand and resolves it through `findProject()`: stored ids are root-qualified, so it accepts either the full id or the bare encoded name, resolving the latter when exactly one root holds that project. A missing operand returns exit `2`; a name held by several roots also returns `2`, listing each candidate id with its Claude directory rather than silently choosing one; an unknown name returns `1`, and a known-but-empty project returns `1` ([src/cli/index.ts#L59-L77](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L59-L77)). Its table renders each session id alongside `formatRelativeTime(s.mtimeMs)` and `formatBytes(s.sizeBytes)`.

Sources: [src/cli/index.ts:L43-L77](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L43-L77)

### `analyze` — single session

`cmdAnalyze` resolves a session reference through `resolveSessionPath`, which treats an argument ending in `.jsonl` or containing `/` as a filesystem path and otherwise looks the id up across all projects via `findSessionById` ([src/cli/index.ts#L79-L95](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L79-L95)). It then runs the core pipeline directly: `parseSessionFile` yields events and parse errors, `loadPricing` supplies the pricing table, and `analyzeSession` produces the `SessionAnalysis` ([src/cli/index.ts#L96-L98](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L96-L98)). With `--json` it prints `JSON.stringify` of the analysis augmented with a `parseErrors` count; otherwise it prints `renderSessionSummary(analysis, { color })` and, when anything was not fully understood, closes with the session's parse coverage — unparseable lines skipped, lines kept as tolerant "unknown" events, and the total line count. Color is enabled only when stdout is a terminal and `NO_COLOR` is absent, so redirected reports remain plain text. Unlike `stats` and `serve`, `analyze` reads and parses the raw `.jsonl` file and needs no index.

The `--with-claude` flag replaces the rendered report with a **Claude Code retrospective** of the session: `cmdAnalyzeWithClaude` builds a grounded prompt (the session's own metrics plus a pointer to the transcript) and streams `runClaudeAnalysis(...)` — a headless `claude -p` run — to stdout, printing the run's own cost to stderr at the end (`src/core/claude-handoff.ts`). It points Claude at the `.jsonl` **read-only** (`--allowedTools Read`, never `--resume`) and uses the user's normal Claude Code login (not `--bare`), so it works for subscription users; the optional handoff may send session content to the model provider and may run normal hooks/configuration. `--model <id>` overrides the default (`sonnet`, persisted as the `analysisModel` preference); an invalid model exits `2`, a missing `claude` binary exits `1`, and `--json` combined with `--with-claude` exits `2` (streamed prose and JSON are mutually exclusive). A dedicated `takeFlagValue` helper strips `--model <value>` before the positional filter runs, so the value can't be mistaken for the session id.

`analyze` also drives the **shareable export** (`src/core/session-markdown.ts`): `analyze <id|path> --md` prints paste-ready Markdown, `--html` prints a standalone HTML file (inline dark theme, print stylesheet, `sanitizeFilename` for the `sessionId`), and `--json` remains the raw `SessionAnalysis`. The three are mutually exclusive (`--md` ∧ `--html` ∧ `--json` ⇒ exit `2`, likewise any ∧ `--with-claude`), and `--md`/`--html` are the only formats that render the full Markdown/HTML builder — `--json` bypasses it. `--out <file|dir>` writes instead of printing: a directory auto-names `cc-analyzer-<sanitizedId>.*` via `sanitizeFilename` + `path.join/extname`, a bare file without an extension gains the inferred one, and the parent directory is created; `Bun.write` errors (EACCES/ENOSPC) are caught and reported as `error: cannot write …` with exit `1`. `--redact` hides prompt and transcript text and rolls `title`/`projectPath` to `[redacted]` and `filesTouched`/`bashCommands` to counts, and `--include-transcript` appends the transcript appendix — off by default, capped at `600` items × `2000` chars (the builder also samples the turns table at `300` and chart tables at `300`), with a `… N more truncated` note and, in the CLI, a `note: transcript truncated to 600 of …` on stderr. `--redact`/`--include-transcript` without `--md/--html/--json` exit `2`. All three export paths compute the same `whatIf` (`sessionWhatIf`), `health` (`inspectSessionHealth`), `rank` (`sessionCostRank`, null when unindexed) and `costBasis` (`getCostBasis`) and thread them into `buildSessionMarkdown`/`buildSessionHtml` — the same bun-free builders the web `GET /api/sessions/:id/report` and the TUI `export` mode call, so the artifacts are byte-identical for the same flags. See [Recipes & Use Cases](./10-recipes.md) for redacted team shares and post-mortems.

Sources: [src/cli/index.ts:L79-L107](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L79-L107) [src/core/session-markdown.ts:L1-L40](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/session-markdown.ts#L1-L40) [src/core/session-markdown.ts:L46-L180](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/session-markdown.ts#L46-L180)(<https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L79-L107>)
...
Sources: [src/cli/index.ts:L109-L130](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L109-L130)

### `stats` — portfolio analytics

`cmdStats` builds the shared portfolio shape with `buildPortfolioStats(db, localDayOfMs(Date.now()))` — the same builder that backs the `/api/stats` web endpoint — and returns `1` when the index is empty ([src/cli/index.ts#L132-L141](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L132-L141)). It then layers terminal-only extras on top: `cacheTtlSplit`, the top ten `analytics.bash` rows, `analytics.tests`, `analytics.retries`, and a `concurrency` headline of `peak` and `parallelDayShare` ([src/cli/index.ts#L142-L153](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L142-L153)). The composite `view` prints as raw JSON under `--json` or through `renderStats(view, { color })` otherwise. As with `analyze`, styling is TTY-only and honors `NO_COLOR`, leaving pipes and redirected output ANSI-free. The rich analytics behind this command are documented on the [Analytics and Insights](./7-analytics-and-insights.md) page.

Sources: [src/cli/index.ts:L132-L156](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L132-L156)
...
Sources: [src/cli/index.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts) [src/core/inventory.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/inventory.ts) [src/core/setup-audit.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/setup-audit.ts)
...
Sources: [src/cli/index.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts) [src/core/portfolio-diagnostics.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/portfolio-diagnostics.ts) [src/core/portfolio-signals.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/portfolio-signals.ts)
...
Sources: [src/cli/index.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts) [src/core/digest.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/digest.ts) [src/core/digest-signals.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/digest-signals.ts)

### `export` — portfolio, project, and session bundles

`cc-analyzer export` writes a folder containing JSON, CSV, Markdown, and/or HTML
artifacts. Select a portfolio (default), `--project <id>`, or `--session <id|path>`;
choose formats with `--format json,csv,md,html|all`; set `--redact` for a
shareable tree, `--split` for both private and shareable trees, and
`--include-transcript` only when needed. `--zip` creates an archive and requires
the system `zip` command. Use `cc-analyzer help` for the complete flag list;
`export --help` is not a separate command.

The export builders are shared with the web bulk-export endpoint and the
single-session report paths. See [Export and share](/guide/export-share) for
user workflows and privacy guidance.
...
The `serve` branch parses `--port=` and `--host=`, plus `--refresh` to run an incremental index update before binding and `--open` to launch the browser after binding. Browser launch is best-effort and restricted to loopback hosts. Invalid ports return exit code `2`, and the web server is dynamically imported so web dependencies stay out of other command startup paths. `pricing update` accepts only the `update` sub-token; `cmdPricingUpdate` forces a refresh with `loadPricing({ force: true })` and returns `1` when the source is not `remote`, meaning the remote fetch failed and a cached or bundled table is still in use ([src/cli/index.ts#L158-L170](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L158-L170)). `cmdUpdate` handles `--check` by comparing `fetchLatestVersion` against `VERSION`, and otherwise runs `performUpdate` with a TTY-only progress callback that writes megabyte counts to `stderr` ([src/cli/index.ts#L172-L204](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L172-L204)). The update mechanics live on the [Updates and Distribution](./8-updates-and-distribution.md) page.

Sources: [src/cli/index.ts:L158-L204](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L158-L204) [src/cli/index.ts:L224-L246](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L224-L246)

### Passive update notice

After `runCommand` returns, `main` fires a best-effort, non-blocking update notice for a curated set of quick commands. `NOTIFY_COMMANDS` contains `projects`, `sessions`, `analyze`, `doctor`, `index`, `stats`, `audit`, `insights`, `report`, and `pricing`; when the command is in that set and `--json` was not passed, `main` awaits `maybeNotifyUpdate()` before returning the exit code ([src/cli/index.ts#L206-L276](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L206-L276)). Excluding `--json` keeps machine-readable output clean of the human-facing banner. Before process exit, `main` also awaits the bounded `flushTelemetry()` drain; it never changes the command result and waits at most briefly for already-pending events. On the normal path nothing is pending and it returns immediately, because a quick command's telemetry event is delivered by a **detached child process** rather than by the parent: `process.exit()` would otherwise kill the socket long before a cold TLS handshake completes. `trackCommand` re-invokes the executable with the hidden `__telemetry-post` marker, the endpoint, and the prebuilt event body; the drain covers only the in-process fallback used when that spawn is refused. The marker is absent from `HELP`, is never itself tracked, prints nothing, and re-checks the opt-out before sending.

Sources: [src/cli/index.ts:L206-L277](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L206-L277)

### Formatting primitives

[src/cli/format.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/format.ts) holds pure, dependency-free formatters. The money/count/duration primitives — `formatUSD` (small non-zero amounts to four decimals, everything else to two, sign preserved), `formatCount` (compacts large numbers into `k`/`M`/`B` suffixes, bucketing on the rounded value so `999_960` renders as `1.0M` rather than `1000.0k`), plus `formatSignedCount`, `formatDuration`, and `formatCompactDuration` — are defined once in the bun-free [src/core/format-shared.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/core/format-shared.ts) and **re-exported** here, so the terminal, the digest markdown, and the web app print identical numbers. `format.ts` itself defines the terminal-only helpers: `formatTokens` appends a `+N cache` suffix when cache tokens are present, `formatBytes` and `formatRelativeTime` cover sizes and human-relative timestamps ([src/cli/format.ts](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/format.ts)). `table` computes per-column widths from headers and rows, then emits a padded header, dashed separator, and rows; its optional `align` array right-aligns numeric columns while text remains left-aligned. `truncate` collapses whitespace and appends an ellipsis past a max length.

Sources: [src/cli/format.ts:L1-L66](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/format.ts#L1-L66)

### Report renderers

`renderSessionSummary` turns a `SessionAnalysis` into a multi-section text report: a branded title and muted identity line; a Totals table covering cost, turns, API calls, tool calls, tokens, duration, active time, web search/fetch, subagent spend, test runs, tool-call churn, a `corrections` line when the session had any correction or interruption turns (e.g. `3 correction turns (18%) · 1 interruption`, followed by the shared `CORRECTION_CAVEAT`), and — only when a signal is non-trivial (a failing-test streak ≥ 3 or ≥ 4 redundant reads) — one `thrash` line. **Actionable diagnostics** follows immediately after Totals (and its correction caveat), so the session's next-step findings are never scrolled away by the reference tables below — before Cost by token category, Cost-per-outcome (per turn / per file touched / per test run / per active hour, only ratios whose denominator exists, closed by the shared `OUTCOME_CAVEAT`), Models, a What-if repricing table when the caller passed one in via `options.whatIf` (computed in `cmdAnalyze` from `sessionWhatIf` — the renderer never sees the pricing table — closed by the shared `WHATIF_CAVEAT`), Tools, a Skills table (uses, attributed turns, and the turn-scoped `turn $` cost, closed by the shared `SKILL_COST_CAVEAT`), and a Subagent-bursts table when the session spawned subagents (per-burst type, turn, calls, and cost, with a best-effort-attribution caveat line). A **▸ Session facts** section then collects `Subagents:` (kept even when the burst join fails to attribute types), `Files touched:`, `Stop reasons:`, `Permission modes:` (present only when a mode other than plain `default` is present), and `Shell commands:` as prose lines rendered `Bash 412 · Read 298 · Edit 150` (not the machine-facing `Bash:412, Read:298`) — the section header itself is omitted when none of the five apply. Finally the per-turn Turns table adds a `flags` column built from the shared `turnFlags()` predicate (interrupted / correction / retries / test failures / redundant reads / tool errors, joined `interrupted · 2 retries`) via `buildTurnSeries`, carries a `shape` column from the shared `turnCostShape()` predicate (`subagent` / `cache churn` / `generation` / `long context`, blank when no component dominates), closed by a muted legend line spelling out each shape actually on screen, a `share` column (that turn's share of the session's cost, through the shared guarded `shareOf` — a $0 session reads `0%`, never `NaN%`), caps at 40 rows, and — when the cap actually bites — shows the **costliest** 40 turns rather than the first 40, adding a `cum` running-share column (`cumulativeShares`) so "these 5 turns are 61% of the spend" reads straight off the table, retitling the section `Turns · top 40 by cost` and printing a muted "Ranked by cost, not session order — N cheaper turns not shown; use --json for the full timeline." line. A session that fits under the cap keeps its plain `Turns` heading and session order, since the whole narrative is on screen either way. A **Context growth** section follows the turns table when the session grew its context at all: the shared bun-free `buildContextGrowth()` attributes each main-chain prompt-side increase (`promptTokens(N+1) − promptTokens(N) − outputTokens(N)`) to the steps the *previous* call issued, ranked by size and capped at 10 rows, with the attributed total, the count of smaller contributors omitted, the count of call pairs skipped across a compaction, and the shared `CONTEXT_GROWTH_CAVEAT` printed verbatim. It reports a **token delta only** — no dollar figure is derived, because cache TTL expiry and later compactions both break the multiplication that would produce one. Section markers and numeric alignment match the portfolio report, while color remains an injected rendering option rather than a core-data concern.
...
Sources: [src/cli/render.ts:L19-L315](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/render.ts#L19-L315)

## Configuration & Extension Points

| Flag | Command | Purpose |
| ---- | ------- | ------- |
| `--json` | `analyze`, `doctor`, `stats`, `audit`, `insights`, `report` | Emit a command-specific machine-readable payload instead of a rendered report; also suppresses the passive update notice |
...
| `--md` | `analyze`, `report` | `analyze --md` prints the session as paste-ready Markdown via `src/core/session-markdown.ts` (health/what-if/rank/costBasis embedded, turns/charts sampled, transcript omitted unless `--include-transcript`); `report --md` prints the digest. `--md` ∧ `--html` ∧ `--json` mutually exclusive (any two ⇒ exit `2`); likewise any ∧ `--with-claude` |
| `--html` | `analyze` | `analyze --html` prints the same builder as a standalone single-file HTML (inline dark theme, `sanitizeFilename`-safe `sessionId`); mutually exclusive with `--md`/`--json` |
| `--json` | `analyze`, `doctor`, `stats`, `audit`, `insights`, `report` | For `analyze --json`, the payload includes derived health, what-if, rank, cost-basis, and coverage fields; with `--redact`, sensitive fields are redacted and transcript content is capped at `600×2000` |
| `--out <file\|dir>` | `analyze` | Write the Markdown/HTML/JSON export to a file instead of stdout; a directory auto-names `cc-analyzer-<sanitizedId>.*`, a bare name without an extension gains the inferred one, parent dirs are created; requires `--md`/`--html`/`--json`, bare `--out` ⇒ exit `2`. `Bun.write` EACCES/ENOSPC ⇒ `error: cannot write …` exit `1` |
| `--redact` | `analyze` | Hide prompt/transcript text and PII in Markdown/HTML/JSON exports (`title`/`projectPath` → `[redacted]`, `filesTouched`/`bashCommands` → counts); requires `--md`/`--html`/`--json`, bare ⇒ exit `2`. Mirrors `sanitizeFilename` + the same builder the web/TUI call |
| `--include-transcript` | `analyze` | Append the transcript appendix to Markdown/HTML/JSON exports — off by default, capped at `600` items × `2000` chars with a `… N more truncated` note (and a `note: truncated to 600 of …` on stderr); requires `--md`/`--html`/`--json`, bare ⇒ exit `2` |
...
Sources: [src/cli/index.ts:L209-L246](https://github.com/yorch/cc-analyzer/blob/51ce1a4/src/cli/index.ts#L209-L246)
