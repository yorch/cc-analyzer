...
`cmdClaudeDir` reads or writes the persisted `claudeDirs` preference (`src/core/prefs.ts`), the third of the five tiers `claudeRoots()` resolves. `cc-analyzer claude-dir` with no argument (or `show`) prints every resolved directory, *the tier it came from*, and a marker on any that holds no `projects/` directory. It suggests configuring something only when **no** resolved directory is readable — the default `~/.claude` and an inherited `CLAUDE_CONFIG_DIR` are both working setups that persist nothing, and prompting those users would read as a fault report. The tier line — `--claude-dir`, `CC_ANALYZER_CLAUDE_DIR`, `cc-analyzer claude-dir`, `CLAUDE_CONFIG_DIR`, or `default` — which is what makes an empty portfolio diagnosable rather than mysterious. `set <path>` replaces the persisted list, `add <path>` appends, `remove <path>` drops one (exit `1` if it was not persisted), and `reset` clears the preference so resolution falls back through `CLAUDE_CONFIG_DIR` to `~/.claude`. Paths are expanded (`~`) and made absolute before storage; a path that does not exist yet is a *warning*, not an error, since a synced or mounted directory can legitimately be absent right now. Every mutating form closes by printing the newly resolved list and reminding the user to run `cc-analyzer index`, because the index mirrors the configured set — a de-configured directory's rows are pruned on the next scan. A bad subcommand or a missing operand exits `2`. Writes touch only `<stateDir>/prefs.json`; no Claude directory is ever modified.
