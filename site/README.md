...
(<https://cc-analyzer.brnby.com/>): a landing page, user-oriented guides, and
the detailed implementation reference wiki.
...
`/wiki` (repo root) is the **canonical** source of the implementation reference,
regenerated by the `repo-wiki-generator` skill. `scripts/sync-wiki.ts` copies it into `site/docs/`
...
User guides live under `site/guide/` and are hand-authored; they are not copied
from the wiki. The docs sidebar is defined by hand in `.vitepress/config.ts`. If
the wiki is regenerated with a **different set of pages**, update that sidebar
to match.
