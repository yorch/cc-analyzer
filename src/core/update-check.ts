  ...
  try {
    ...
    if (!cache || isStale(cache.lastCheck, now)) {
      try {
        ...
        // Offline or slow — fall back to the cached value, if any, and still
        // advance lastCheck so an offline user doesn't pay the 1s timeout on
        // every subsequent command for the rest of the day.
        await writeCache(path, { lastCheck: now, latest: latest ?? VERSION });
      }
    }
